...
export default function Contact() {
  return (
    <MotionSection id="contact" className="bg-porcelain py-24">
      <div className="section-shell">
        <div className="grid gap-12 lg:grid-cols-[0.95fr_1.05fr]">
          <div>
            <p className="text-sm font-bold tracking-[0.24em] text-gold">CONTACT</p>
            <h2 className="mt-5 text-4xl font-semibold leading-tight text-ink md:text-5xl">방문 전, 동선과 시간을 먼저 정리해 드립니다.</h2>
            <p className="mt-6 text-base leading-8 text-ink/65">
              초진은 상태 확인과 움직임 평가 시간이 포함될 수 있습니다. 정확한 시간은 접수 후 상담을 통해 안내됩니다.
            </p>

            <div className="mt-9 grid gap-3 sm:grid-cols-2">
              {contactCards.map(({ icon: Icon, title, text }) => (
                <div key={title} className="rounded-3xl border border-mist bg-white p-5 shadow-soft">
          ...
          <div className="rounded-[2rem] border border-mist bg-white p-5 shadow-clinic">
            <div className="grid min-h-[330px] place-items-center rounded-[1.5rem] bg-[linear-gradient(135deg,#e8eeec,#f7f4ee)] text-center">
              <div className="px-8">
                <MapPin className="mx-auto text-navy" size={38} />
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              <div className="rounded-3xl bg-clinic p-5">
              ...
              <div className="rounded-3xl bg-navy p-5 text-white">
                ...
                <a href={`tel:${clinic.phone}`} className="focus-ring mt-5 inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-semibold text-navy">
                  바로 전화하기
                  <Navigation size={15} />
                </a>
              </div>
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2">
          {assurance.map(({ icon: Icon, title, description }) => (
            <div key={title} className="rounded-3xl border border-mist bg-white p-6">
